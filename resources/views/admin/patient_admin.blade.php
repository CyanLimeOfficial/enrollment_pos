...
<body>
                                ...
                                $(document).ready(function() {
                                    ...
                                    $('#previewBtn').on('click', function() {
                                        ...
                                        selectedRows.forEach((row) => {
                                            let rowHtml =
                                                `<tr data-health-record-id="${row.health_record_id}" data-dependent-id="${row.dependent_hospital_id}">`;
                                            rowHtml +=
                                                `<td class="category-cell" data-index="0" style="cursor:pointer;">POS</td>`;
                                    ...
                                    // 3. Delegate click event for category cell cycling
                                    ...
                                    $('#selectAll').on('change', function() {
                                        dataTable.rows({
                                            page: 'all'
                                        }).nodes().to$().find('.rowCheckbox').prop('checked', this.checked);
                                    });

                                    // 5. Handle Export to Excel Button Click
                                ...
                                <table id="selectable"
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <div>
                                            ...
                                            <button id="selectAbove51Days" class="btn"
    ...
    <script>
        ...
        var isAbove51DaysActive = false; // Track selection state

        document.getElementById("selectAbove51Days").addEventListener("click", function() {
            // Get the already-initialized DataTable instance.
            let table = $('#selectable').DataTable();
            // Toggle state.
            isAbove51DaysActive = !isAbove51DaysActive;

            // Loop over every row (visible or not)
            table.rows().every(function() {
                let row = this.node();
                // Get the 4th cell (index 3) using standard DOM API.
                let daysLeftCell = row.cells[3];
                let checkbox = row.querySelector(".rowCheckbox");

                if (daysLeftCell && checkbox) {
                    // Parse cell text as an integer (base 10).
                    let daysText = daysLeftCell.textContent.trim();
                    let daysLeft = parseInt(daysText.replace(/\D/g, ""), 10);
                    // When active, check the checkbox if daysLeft is 51 or above.
                    checkbox.checked = isAbove51DaysActive ? (daysLeft >= 51) : false;
                }
            });

            // Update button text and styling.
            let btn = document.getElementById("selectAbove51Days");
            if (isAbove51DaysActive) {
                btn.textContent = "Deselect Above 51 Days";
                btn.style.backgroundColor = "green"; // Active state
                btn.style.color = "white";
            } else {
                btn.textContent = "Select Above 51 Days";
                btn.style.backgroundColor = "lightgreen"; // Default state
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let selectAbove51DaysBtn = document.getElementById("selectAbove51Days");
            ...
            // Additional click listener to toggle the active class and styles.
            selectAbove51DaysBtn.addEventListener("click", function() {
                let isActive = selectAbove51DaysBtn.classList.contains("active");

                if (!isActive) {
                    selectAllBtn.disabled = true;
                    selectAbove51DaysBtn.classList.add("active");
                    selectAbove51DaysBtn.style.backgroundColor = "green";
                    selectAbove51DaysBtn.style.color = "white";
                } else {
                    selectAllBtn.disabled = false;
                    selectAbove51DaysBtn.classList.remove("active");
                    selectAbove51DaysBtn.style.backgroundColor = "lightgreen";
                    selectAbove51DaysBtn.style.color = "black";
                }
            });
        });
    </script>
